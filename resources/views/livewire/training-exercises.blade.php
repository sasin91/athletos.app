<div class="relative">
    ...
    <fieldset id="exercise-{{ $exercise->exerciseSlug }}" class="exercise-fieldset bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6" x-data="{ showAlternatives: false }">
            ...
            @for($set = 1; $set <= $exercise->sets; $set++)
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    ...
                    <div>
                        ...
                        <input type="number"
                            ...
                            wire:blur="completeSet('{{ $exercise->exerciseSlug }}', {{ $set }})"
                            min="0"
                            max="50"
                    ...
                    <div>
                        ...
                        <x-exercise-weight-suggestion class="mt-2" :suggested="$suggestedExerciseWeights[$exercise->exerciseSlug] ?? null" :hasHistory="!empty($previousExerciseWeights[$exercise->exerciseSlug] ?? [])" />
                    </div>
                    ...
                    <div>
                        ...
                        <select wire:model.live.debounce.500ms="completedSets.{{ $exercise->exerciseSlug }}.{{ $set }}.rpe"
                            wire:change="completeSet('{{ $exercise->exerciseSlug }}', {{ $set }})"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:text-gray-100">
                            <option value="">-</option>
                ...
                <div>
                    ...
                    <textarea wire:model.live="exerciseNotes.{{ $exercise->exerciseSlug }}"
                        wire:change="addNotes('{{ $exercise->exerciseSlug }}')"
                        rows="2"
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:text-gray-100"
