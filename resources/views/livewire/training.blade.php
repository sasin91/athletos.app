        ...
        <div class="mt-4">
            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3">
                <div class="bg-blue-600 dark:bg-blue-400 h-3 rounded-full transition-all duration-300"
                    x-bind:style="`width: {{ $training->progress }}%`" role="progressbar"
                    aria-valuenow="{{ $training->progress }}" aria-valuemin="0" aria-valuemax="100">
                </div>
            </div>
        </div>
    </header>
    ...
    <div class="relative">
        @if (count($this->plannedExercises) > 0)
            ...
            @if (count($this->plannedExercises) > 1)
                <nav
                    class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 mb-8">
            ...
            @foreach ($sets as $exerciseSlug => $exerciseSets)
                <fieldset id="exercise-{{ $exerciseSlug }}"
                    class="mb-8 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
                    <legend class="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-6 flex items-center gap-3">
                        <span
                            class="inline-flex items-center justify-center w-8 h-8 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400 rounded-full text-sm font-medium">
                            {{ $loop->iteration }}
                        </span>
                    ...
                    @foreach ($exerciseSets as $set)
                        <div x-data="{ focused: false }"
                            :class="focused ? 'bg-blue-50 dark:bg-blue-900/30 shadow-lg ring-2 ring-blue-400' :
                                'bg-white dark:bg-gray-800 shadow-sm'"
                            class="mb-6 rounded-lg transition-all duration-200 p-6">
                            <!-- Set Header -->
                            <div class="flex items-center justify-between mb-4">
                                <div class="text-lg font-semibold text-gray-700 dark:text-gray-300">Set {{ $set->setNumber }}</div>
                                <button type="button"
                            ...
                            <div class="space-y-4 mb-4">
                                <div class="relative">
                                    <label for="reps-{{ $exerciseSlug }}-{{ $set->setNumber }}"
                                        class="absolute -top-2 left-2 inline-block rounded-lg bg-white dark:bg-gray-800 px-1 text-xs font-medium text-gray-900 dark:text-gray-100">Reps</label>
                                    <input type="number" name="reps-{{ $exerciseSlug }}-{{ $set->setNumber }}"
                                        id="reps-{{ $exerciseSlug }}-{{ $set->setNumber }}"
                                        class="block w-full rounded-md bg-white dark:bg-gray-900 px-3 py-2 text-base text-gray-900 dark:text-gray-100 outline-1 -outline-offset-1 outline-gray-300 dark:outline-gray-700 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
                                        placeholder="Reps" @focus="focused = true" @blur="focused = false"
                                        wire:model.defer="sets.{{ $exerciseSlug }}.{{ $loop->index }}.reps"
                                        value="{{ $set->reps }}" />
                                </div>
                                <div class="relative">
                                    <label for="weight-{{ $exerciseSlug }}-{{ $set->setNumber }}"
                                        class="absolute -top-2 left-2 inline-block rounded-lg bg-white dark:bg-gray-800 px-1 text-xs font-medium text-gray-900 dark:text-gray-100">Weight</label>
                                    <input type="text" name="weight-{{ $exerciseSlug }}-{{ $set->setNumber }}"
                                        id="weight-{{ $exerciseSlug }}-{{ $set->setNumber }}"
                                        class="block w-full rounded-md bg-white dark:bg-gray-900 px-3 py-2 text-base text-gray-900 dark:text-gray-100 outline-1 -outline-offset-1 outline-gray-300 dark:outline-gray-700 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
                                        placeholder="Weight (kg)" @focus="focused = true" @blur="focused = false"
                                        wire:model.defer="sets.{{ $exerciseSlug }}.{{ $loop->index }}.weight"
                                        value="{{ $set->weight }}" />
                                </div>
                                <div class="relative">
                                    <label for="rpe-{{ $exerciseSlug }}-{{ $set->setNumber }}"
                                        class="absolute -top-2 left-2 inline-block rounded-lg bg-white dark:bg-gray-800 px-1 text-xs font-medium text-gray-900 dark:text-gray-100">RPE</label>
                                    <input type="number" name="rpe-{{ $exerciseSlug }}-{{ $set->setNumber }}"
                                        id="rpe-{{ $exerciseSlug }}-{{ $set->setNumber }}"
                                        class="block w-full rounded-md bg-white dark:bg-gray-900 px-3 py-2 text-base text-gray-900 dark:text-gray-100 outline-1 -outline-offset-1 outline-gray-300 dark:outline-gray-700 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
                                        placeholder="RPE" @focus="focused = true" @blur="focused = false"
                                        wire:model.defer="sets.{{ $exerciseSlug }}.{{ $loop->index }}.rpe"
                                        value="{{ $set->rpe }}" />
                                </div>
                            </div>
                            ...
                            <div class="border-t border-gray-200 dark:border-gray-600 pt-4">
                                <div class="flex items-center justify-center" x-data="{
                                    ...
                                    <div class="text-center">
                                        <div class="text-sm text-gray-600 dark:text-gray-400 mb-2">Rest Timer</div>
                                        <div class="font-mono text-xl mb-3"
                                            x-text="`${String(Math.floor(restSeconds/60)).padStart(2, '0')}:${String(restSeconds%60).padStart(2, '0')}`">
                                        </div>
                                        <div class="flex gap-2 justify-center">
                                            <button type="button" @click="startRest()" x-show="!restRunning"
                                                class="px-3 py-1 text-sm bg-green-500 text-white rounded-md hover:bg-green-600 transition-colors">Start</button>
                                            <button type="button" @click="stopRest()" x-show="restRunning"
                                                class="px-3 py-1 text-sm bg-red-500 text-white rounded-md hover:bg-red-600 transition-colors">Stop</button>
                                            <button type="button" @click="resetRest()"
                                                class="px-3 py-1 text-sm bg-gray-500 text-white rounded-md hover:bg-gray-600 transition-colors">Reset</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    @endforeach
                    <button type="button" wire:click="addSet('{{ $exerciseSlug }}')" wire:loading.attr="disabled"
                        wire:loading.class="opacity-50 cursor-not-allowed" wire:target="addSet('{{ $exerciseSlug }}')"
            ...
            @if (count($this->plannedExercises) > 0)
                <fieldset id="training-feedback"
                    class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mt-8">
                    <legend class="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-6 flex items-center gap-3">
                        <span
                            class="inline-flex items-center justify-center w-8 h-8 bg-green-100 dark:bg-green-900 text-green-600 dark:text-green-400 rounded-full text-sm font-medium">
                            ✓
                        </span>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="flex justify-center space-x-2">
                                @for ($rating = 1; $rating <= 5; $rating++)
                                    <button type="button" wire:click="$set('overallRating', {{ $rating }})"
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
                                @foreach (['terrible', 'bad', 'okay', 'good', 'excellent'] as $moodOption)
                                    <button type="button" wire:click="$set('mood', '{{ $moodOption }}')"
                                        ...
                                        <span
                                            class="text-xs font-medium text-gray-700 dark:text-gray-300 capitalize">{{ $moodOption }}</span>
                                    </button>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-5 md:grid-cols-10 gap-2">
                                @for ($level = 1; $level <= 10; $level++)
                                    <button type="button" wire:click="$set('energyLevel', {{ $level }})"
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                @foreach (['too_easy', 'just_right', 'challenging', 'too_hard'] as $diffOption)
                                    <button type="button" wire:click="$set('difficulty', '{{ $diffOption }}')"
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-5 md:grid-cols-10 gap-2">
                                @for ($level = 1; $level <= 10; $level++)
                                    <button type="button" wire:click="$set('difficultyLevel', {{ $level }})"
                        ...
                        <div class="pt-6 border-t border-gray-200 dark:border-gray-600">
                            <button type="button" wire:click="completeTraining"
        ...
        @elseif($hasError)
            <!-- Error State -->
            <div
                class="bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800 p-8 text-center">
        ...
        @else
            ...
            <div
                ...
                <p class="text-gray-500 dark:text-gray-400">This training session doesn't have any exercises planned
                    yet.</p>
            </div>
        @endif
        ...
        <div x-data="{ addingExercise: $wire.entangle('addingExercise') }">
            ...
            <div x-show="addingExercise" x-transition:enter="transition-opacity ease-linear duration-300"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                x-transition:leave="transition-opacity ease-linear duration-200"
                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                class="fixed inset-0 z-40 bg-gray-500/75" aria-hidden="true"
                wire:click="$set('addingExercise', false)" style="display: none;"></div>
            <!-- Drawer (slides in/out) -->
            <div x-show="addingExercise"
                x-transition:enter="transform transition ease-in-out duration-500 sm:duration-700"
                x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0"
                x-transition:leave="transform transition ease-in-out duration-500 sm:duration-700"
                x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full"
                class="fixed inset-y-0 right-0 z-50 flex max-w-full pl-10 sm:pl-16" style="display: none;">
                <div class="pointer-events-auto w-screen max-w-md">
                    <div class="flex h-full flex-col overflow-y-auto bg-white dark:bg-gray-900 shadow-xl">
                        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                            <div class="flex items-start justify-between">
                                <h2 class="text-base font-semibold text-gray-900 dark:text-gray-100"
                                    id="drawer-title">Add Exercise</h2>
                                <div class="ml-3 flex h-7 items-center">
                                    <button type="button" wire:click="$set('addingExercise', false)"
                                        class="relative rounded-md bg-white dark:bg-gray-900 text-gray-400 hover:text-gray-500 focus-visible:ring-2 focus-visible:ring-indigo-500">
                        ...
                        <ul role="list"
                            class="flex-1 divide-y divide-gray-200 dark:divide-gray-700 overflow-y-auto px-6">
                            @foreach ($this->availableExercises as $exercise)
                                <li class="flex justify-between gap-x-6 py-5">
                                    <div class="flex min-w-0 gap-x-4">
                                        <img class="size-12 flex-none rounded bg-gray-50 object-cover"
                                            src="{{ $exercise->image ?? '/images/exercise-placeholder.png' }}"
                                            alt="{{ $exercise->displayName }}" />
                                        <div class="min-w-0 flex-auto">
                                            <p class="text-sm font-semibold text-gray-900 dark:text-gray-100">
                                                {{ $exercise->displayName }}</p>
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                                {{ $exercise->summary ?? '' }}</p>
                                            <div class="mt-1 flex flex-wrap gap-1">
                                                @foreach ($exercise->tags ?? [] as $tag)
                                                    <span
                                                        class="inline-block bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded px-2 py-0.5 text-xs">{{ $tag }}</span>
                                                @endforeach
                                            </div>
                                        </div>
                                    </div>
                                    <div class="flex items-center">
                                        <button type="button"
                                            wire:click="addExercise('{{ $exercise->exercise->value }}')"
                                            class="inline-flex items-center rounded-md bg-indigo-600 px-3 py-1.5 text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600">Add</button>
                                    </div>
                                </li>
                            @endforeach
                        </ul>
                        <div class="p-6 border-t border-gray-200 dark:border-gray-700 flex justify-end">
                            <button type="button" wire:click="$set('addingExercise', false)"
                                class="inline-flex items-center rounded-md bg-gray-200 dark:bg-gray-700 px-4 py-2 text-sm font-semibold text-gray-700 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600">Cancel</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="fixed bottom-0 left-0 w-full z-50 bg-gray-900/95 text-white py-3 shadow-lg">
                <div class="max-w-4xl mx-auto flex items-center justify-between px-4">
                    ...
                    <div class="relative group">
                        ...
                        <div
                            class="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 px-2 py-1 bg-gray-900 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap">
                            Back to Dashboard
                        </div>
                    </div>
                    <!-- Total Timer -->
                    <div x-data="totalTimer()" x-init="init()" class="flex flex-col items-center">
                    ...
                    <!-- Add Exercise -->
                    <div class="relative group">
                        <button type="button" wire:click="$set('addingExercise', true)"
                            class="inline-flex items-center justify-center w-12 h-12 rounded-md bg-indigo-600 text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600">
                            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 4v16m8-8H4" />
                            </svg>
