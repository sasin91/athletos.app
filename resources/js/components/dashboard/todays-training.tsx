...
export default function TodaysTraining({ trainingPlan, isToday, formattedDate, plannedExercises, currentStreak, onStartTraining }: TodaysTrainingProps) {
    return (
        <Card>
            <CardHeader>
                <div className="flex items-center justify-between">
                    <CardTitle className="text-lg">{trainingPlan.name}</CardTitle>
                    <p><em>{formattedDate}</em></p>
                </div>
                {currentStreak > 0 && <Badge variant="secondary">🔥 {currentStreak} day streak</Badge>}
            </CardHeader>
            <CardContent>
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            ...
                            {plannedExercises.length > 3 && (
                                <div className="text-sm text-gray-500 dark:text-gray-400">+{plannedExercises.length - 3} more exercises</div>
                            )}
                        </div>

                        {isToday && (
                            <Button onClick={onStartTraining} className="w-full" size="lg">
                                Start Training
                            </Button>
                        )}
                    </div>
