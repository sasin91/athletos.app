...
async fn openapi_document_describes_every_route(pool: PgPool) {
    ...
    assert!(doc["paths"]["/v1/enrollments/{id}/exercise-adjustments"]["get"].is_object());
    assert!(doc["paths"]["/v1/enrollments/{id}/exercise-adjustments"]["put"].is_object());
...
/// Adjustment rows are subordinate to one enrolment, and invalid percentages
/// must remain impossible even for writers that bypass the API.
#[sqlx::test]
async fn enrollment_exercise_adjustments_have_the_transactional_schema(pool: PgPool) {
    let constraints: Vec<(String, String, String, String)> = sqlx::query_as(
        "select conname::text, contype::text, confdeltype::text,
                pg_get_constraintdef(oid)
         from pg_constraint
         where conrelid = 'enrollment_exercise_adjustments'::regclass
         order by conname",
    )
    .fetch_all(&pool)
    .await
    .expect("failed to read adjustment constraints");

    let constraint = |name: &str| {
        constraints
            .iter()
            .find(|(found, _, _, _)| found == name)
            .unwrap_or_else(|| panic!("missing {name}; found {constraints:?}"))
    };

    let (_, kind, _, definition) = constraint("enrollment_exercise_adjustments_pkey");
    assert_eq!(kind, "p");
    assert_eq!(definition, "PRIMARY KEY (enrollment_id, exercise)");

    let (_, kind, delete_action, definition) =
        constraint("enrollment_exercise_adjustments_enrollment_id_fkey");
    assert_eq!(kind, "f");
    assert_eq!(delete_action, "c", "the foreign key must cascade deletes");
    assert!(definition.contains("REFERENCES enrollments(id) ON DELETE CASCADE"));

    let (_, kind, _, definition) = constraint("enrollment_adjustment_nonzero");
    assert_eq!(kind, "c");
    assert!(definition.contains("adjustment_percent <> 0"));

    let (_, kind, _, definition) = constraint("enrollment_adjustment_range");
    assert_eq!(kind, "c");
    assert!(definition.contains("adjustment_percent >= '-50'::integer"));
    assert!(definition.contains("adjustment_percent <= 50"));
}
